...
public class ElasticSearchDataSupplyTests {
  ...
  private void assetAssetsAreEqual(Asset expectedAsset, Asset actualAsset) {
    ...
    for (var i = 0; i < expectedAsset.getTopics().size(); ++i)
      assertEquals(
          expectedAsset.getTopics().get(i).getId(), actualAsset.getTopics().get(i).getId());
  }
  ...
  private SearchResults givenResultsSet(Asset... assets) {
    ...
    for (var asset : assets) expectedResults.addFound(asset);
  ...
  private Asset[] givenResultsForSearch(String searchString, Asset... assets) throws Exception {
    ...
    for (var asset : assets) document.add(toDocument(asset));
    ...
    when(elasticSearch.execute("assets/_search", queryDocument))
        .thenReturn(new ObjectMapper().writeValueAsString(document));
    return assets;
  }
  ...
  private Asset givenAsset(
      ...
      String thumbnailUrl,
      String vendorId,
      AssetVendorRelationshipLevel vendorLevel,
      String... topicIds) {

    var topics =
        Arrays.asList(
            Arrays.stream(topicIds)
                .map(topicId -> new AssetTopic(topicId, null))
                .toArray(AssetTopic[]::new));

    return new Asset(
        id,
        title,
        URI.create(previewUrl),
        URI.create(thumbnailUrl),
        null,
        null,
        topics,
        new AssetVendor(vendorId, null, vendorLevel, 0));
  }
  ...
  private HashMap<String, Object> toDocument(Asset asset) {
    ...
    document.put(
        "topics", asset.getTopics().stream().map(AssetTopic::getId).toArray(String[]::new));
