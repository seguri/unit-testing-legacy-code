...
class DealsOptimizer {
  public void optimize(SearchResults results, AssetAssessments assessments) {
    var highestRelationshipLevelOptional =
        results.getFound().stream()
            .map(a -> a.getVendor().getRelationshipLevel())
            .max(Enum::compareTo);
              ...
              case Gold:
                if (assessments.isAssetDealEligible(asset) || hasProfitMargin(asset, "1", "2"))
                  if (hasProfitMargin(asset, "7", "10"))
                    if (hasTopLine(asset, "1000.00")) results.getHotspot(Deals).addMember(asset);
                break;
              case Silver:
                if (assessments.isAssetDealEligible(asset)
                    && hasProfitMargin(asset, "1", "2")
                    && hasTopLine(asset, "1500")) results.getHotspot(Deals).addMember(asset);
                break;
            }
            break;
          case Partner:
            switch (asset.getVendor().getRelationshipLevel()) {
              case Partner:
                if (hasProfitMargin(asset, "7", "10")) results.getHotspot(Deals).addMember(asset);
                break;
              case Gold:
                if (assessments.isAssetDealEligible(asset) && hasProfitMargin(asset, "1", "2"))
                  if (hasTopLine(asset, "1000")) results.getHotspot(Deals).addMember(asset);
                break;
              case Silver:
                if (assessments.isAssetDealEligible(asset))
  ...
  private boolean hasProfitMargin(Asset asset, String denominator, String numerator) {
    var scaledRevenue =
        asset
            ...
            .getTotalRevenue()
            .getAmount()
            .multiply(new BigDecimal(denominator));
    var scaledRoyalties =
        asset
            .getPurchaseInfoLast30Days()
            .getTotalRoyaltiesOwed()
            .getAmount()
